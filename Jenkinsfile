...
pipeline {
    ...
    stages {
		...
		stage('Run test cases'){
			...
			sqlconnection().eachRow("SELECT CASE WHEN (SELECT COUNT(*) FROM employees_staging GROUP BY employee_id HAVING COUNT(*) > 1) is null THEN 'Passed' else 'Failed' END as output") { row ->
			...
			sqlconnection().eachRow("SELECT CASE WHEN (SELECT COUNT(*) FROM departments_staging GROUP BY department_id HAVING COUNT(*) > 1) is null THEN 'Passed' else 'Failed' END as output") { row ->
			...
			sqlconnection().eachRow("SELECT CASE WHEN (SELECT COUNT(*) FROM jobs_staging GROUP BY job_id HAVING COUNT(*) > 1) is null THEN 'Passed' else 'Failed' END as output") { row ->
